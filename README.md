...
- **工具调用系统** — 内置 6 种实用工具：联网搜索、计算器、时间日期、随机数、UUID 生成、网页抓取，支持多轮 tool calling，每项可单独开关
- **联网搜索** — 支持博查搜索 API 和本地多引擎代理（必应/搜狗/360/百度），tool calling 自动选择
- **LaTeX 公式渲染** — 支持行内 `$...$` 和块级 `$$...$$` 数学公式，基于 KaTeX 渲染
...
**1. 博查搜索（推荐）**
免费注册获取 API Key：[open.bocha.cn](https://open.bocha.cn)，在设置面板中填入即可使用。

**2. 本地代理**
```bash
node server.js
```
启动后支持必应/搜狗/360/百度四个搜索引擎代理。
...
| 存在惩罚 | 存在惩罚系数 | `0.0` |
| 历史上限 | 包含的上下文轮数 | `20` |
| 深度思考 | 启用推理模型的思维链输出 | `开启` |
| 计算器 / 时间日期 / 随机数 / UUID / 网页抓取 | AI 可用工具集 | `全部开启` |
| 联网搜索 | 启用互联网搜索能力 | `开启`（需配置 API Key 或本地代理）|
...
此版本是从 Vanilla JS 迁移到 Vue 3 + TypeScript + Element Plus 的重构版本，已通过迭代测试修复多项问题，主要功能稳定可用。

已知情况：
- KaTeX（~1.8MB）因体积较大被拆分为独立 chunk，首次加载可能较慢
- 搜狗/360/百度搜索引擎存在反爬限制，本地代理方式下仅必应可稳定工作；推荐使用博查搜索 API
